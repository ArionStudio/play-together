...
function LeaderboardsFallback() {
  return (
    <Page>
      <PageHeader
        title="Leaderboards"
        description="Scores are intended to stay isolated by game, mode, and board definition so competitive and custom runs do not mix."
      />
...
function ConnectedLeaderboardsPage() {
  ...
  const catalog = useQuery(api.minesweeper.leaderboardCatalog)
  const [selectedCategoryKey, setSelectedCategoryKey] = useState<string | null>(null)

  useEffect(() => {
    if (!selectedCategoryKey && catalog && catalog.length > 0) {
      setSelectedCategoryKey(catalog[0]!.categoryKey)
    }
  ...
  return (
    <Page>
      <PageHeader
        title="Leaderboards"
        description="Ranked solo categories are separated by canonical board definition and scoring rule. Practice and custom runs stay out of the ranked board."
      />
          ...
          <div className="mt-4 space-y-2">
            {catalog.map((entry) => (
              <button
                ...
                type="button"
                onClick={() => setSelectedCategoryKey(entry.categoryKey)}
                className={[
              ...
              >
                ...
                <span className="font-mono text-xs">{entry.boardKey}</span>
              </button>
            ))}
          </div>
